...
class _OverviewScreenState extends State<OverviewScreen> {
  ...
  @override
  void initState() {
    Provider.of<ChartDataProvider>(context, listen: false).getAssetExpenses();
    super.initState();
  }

  // get the biggest value in map
  int _getHighestValue() => chartData.values
      .reduce((curr, next) => curr > next ? curr : next)
      .toInt();

  List<FlSpot> _getSpots() {
    // print('keys     ${chartData.keys}');
    ...
    return LineChartData(
      ...
      titlesData: FlTitlesData(
          ...
          getTitles: (value) {
            if (value.toInt() % 2 == 0) {
              return DateFormat('MMM').format(
                DateTime(_toDate.year, _toDate.month + value.toInt() + 1),
              );
            }
            return '';
          },
        ...
        leftTitles: SideTitles(
          ...
          getTitles: (value) {
            //TODO
            var intMax = (_getHighestValue() ~/ 1000);
            // intMax = (intMax * 1000) < _getHighestValue() ?
            // print(intMax);
            switch (value.toInt()) {
              case 1:
                return '10k';
